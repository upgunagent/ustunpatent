    ...
    return (
        <>
            <div className="overflow-x-auto rounded-md border max-h-[calc(100vh-200px)] relative">
                <table className="min-w-full divide-y divide-gray-200 border-collapse table-fixed w-max">
                    <thead className="bg-white sticky top-0 z-20 shadow-sm">
                        <tr>
                            <ResizableTh title="Bülten No" initialWidth={100} />
                            <ResizableTh title="Logo" initialWidth={100} />
                            <ResizableTh title="Marka Adı" initialWidth={250} />
                            <ResizableTh title="Marka Sahibi" initialWidth={250} />
                            <ResizableTh title="Başvuru/Dosya No" initialWidth={150} />
                            <ResizableTh title="Başvuru Tarihi" initialWidth={120} />
                            <ResizableTh title="Kayıt Tarihi" initialWidth={120} />
                            <ResizableTh title="Sınıflar" initialWidth={120} />
                            <ResizableTh title="Sınıf İçerikleri" initialWidth={300} />
                            <ResizableTh title="Çıkartılan İçerikler" initialWidth={200} />
                            <ResizableTh title="Viyana Sınıfı" initialWidth={120} />
                        </tr>
                    </thead>
                    <tbody className="bg-white divide-y divide-gray-200">
                        {data.map((mark, index) => (
                            <tr key={index} className="hover:bg-gray-50 divide-x divide-gray-200 group">
                                <td
                                    className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 overflow-hidden text-ellipsis border-gray-200 cursor-pointer active:bg-blue-50"
                                    onClick={(e) => handleCopy(mark.issue_no, e)}
                                >
                                    {mark.issue_no}
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 border-gray-200">
                                    {mark.logo_url ? (
                                        <div
                                            className="relative w-16 h-16 border rounded cursor-pointer overflow-hidden bg-gray-100"
                                            onClick={(e) => {
                                                e.stopPropagation();
                                ...
                                <td
                                    className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 overflow-hidden text-ellipsis max-w-0 cursor-pointer active:bg-blue-50 relative pr-10"
                                ...
                                >
                                    ...
                                    {mark.similarity && mark.similarity.score > 0 && (
                                        <div className="absolute right-2 top-1/2 -translate-y-1/2 group/badge">
                                            <span className={`flex items-center justify-center w-9 h-9 rounded-full ${getSimilarityColor(mark.similarity.score)} text-white text-[10px] font-bold shadow-sm z-10 border-2 border-white`}>
                                ...
                                <td
                                    className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 overflow-hidden text-ellipsis max-w-0 cursor-pointer active:bg-blue-50"
                                ...
                                <td
                                    className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 border-gray-200 cursor-pointer active:bg-blue-50"
...
const ResizableTh = ({ title, initialWidth }: { title: string; initialWidth: number }) => {
    ...
    return (
        <th
            ...
            className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider bg-gray-50 border-r border-gray-200 relative group select-none whitespace-nowrap"
        >
